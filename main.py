# -*- coding: utf-8 -*-
"""
Created on Thu Sep 16 11:21:50 2021
...
def init_wb(nn_list, w_input):
    ...
    if isinstance(w_input, str):
        if w_input == "xavier":
            ...
            raise Exception("Weight initializer undefined: %s" % w_input)

    else:
        raise Exception("Weight initializer undefined")
...
if __name__ == "__main__":
    ...
    # weight
    print("Weights initializer: %s" % str(WEIGHT))
    train_config["weight"] = WEIGHT

    # read train and test npz
    dataset = dict()
    mnist_data = np.load(PATH_MNIST)
    ...
    # img list file
    if args.imglist:
        print("Image list read from: %s" % args.imglist)
        img_list = []
        with open(args.imglist, "r") as fp:
